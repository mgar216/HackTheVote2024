use curve25519_dalek::{digest::Update, RistrettoPoint, Scalar};
...
impl Proof {
    fn generate(ctxt: VoteCiphertext, rng: &mut impl CryptoRngCore) -> Proof {
        ...
        let c = Scalar::from_hash::<Sha512>(
            Sha512::new().chain(xh.to_bytes()).chain(rh.to_bytes()),
        );
        let xcr = ctxt.x * c + r;
        Proof { xh, rh, xcr }
    }
    ...
    fn check(&self) -> Result<(), ()> {
        let c = Scalar::from_hash::<Sha512>(
            Sha512::new().chain(self.xh.to_bytes()).chain(self.rh.to_bytes()),
        );
...
fn main() -> io::Result<()> {
    ...
    let a_vote = -Scalar::from(49u64);
    let b_vote = -Scalar::from(49u64);
    let c_vote = Scalar::from(99u64);

    let ballot = Ballot::new(a_vote, b_vote, c_vote, &mut rng);
